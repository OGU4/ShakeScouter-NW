...
def main() -> None:
	...
	alt_gray = cv.cvtColor(roi_color, cv.COLOR_BGR2GRAY)
	alt_bin = cv.adaptiveThreshold(alt_gray, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 11, 2)
	log_image_stats('wave_alt stats', alt_bin)
	...
	save_debug_image('wave_after_filter', timestamp, wave_image)
	save_debug_image('wave_alt_binarized', timestamp, alt_bin)

	TELEMETRY_DIR.mkdir(parents=True, exist_ok=True)
	frame_path = TELEMETRY_DIR / f'wave_frame_{timestamp}.png'
	roi_color_path = TELEMETRY_DIR / f'wave_roi_color_{timestamp}.png'
	roi_path = TELEMETRY_DIR / f'wave_roi_processed_{timestamp}.png'
	tpl_path = TELEMETRY_DIR / f'wave_template_{timestamp}.png'
