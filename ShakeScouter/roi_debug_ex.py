...
def main() -> None:
	...
	template = Scene.loadTemplate('wave_ex')

	if wave_image.shape != template.shape:
		raise RuntimeError('processed ROI and template shapes differ')

	mae = errorMAE(wave_image, template)

	TELEMETRY_DIR.mkdir(parents=True, exist_ok=True)
	frame_path = TELEMETRY_DIR / f'ex_wave_frame_{timestamp}.png'
	roi_color_path = TELEMETRY_DIR / f'ex_wave_roi_color_{timestamp}.png'
	roi_path = TELEMETRY_DIR / f'ex_wave_roi_processed_{timestamp}.png'
	tpl_path = TELEMETRY_DIR / f'ex_wave_template_{timestamp}.png'
	save_image(frame_path, frame_image)
	save_image(roi_color_path, roi_color)
	save_image(roi_path, wave_image)
	save_image(tpl_path, template)

	result = {
		'frame_shape': list(frame_image.shape),
		'roi_shape': list(wave_image.shape),
		'template_shape': list(template.shape),
		'mae': mae,
		'extra_detected': bool(mae <= EXTRA_THRESHOLD),
